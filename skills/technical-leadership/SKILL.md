...
description: "Use when you own outcomes you don't implement (tech lead, staff, team lead), a technical disagreement is looping, deciding what to delegate, or negotiating scope/debt/deadlines — decide legitimately, grow engineers, keep bus factor > 1."
---
...
4. **Run disagreements as structured divergence-then-commit**: frame the question, solicit positions in writing *before* the meeting (anchoring-resistant — see brainstorming: criteria first, and the quiet people write better than they interrupt), extract the assumptions behind the conflict (see decomposing-ambiguity: two experts disagreeing), decide on the stated clock, record, commit — and then *watch for relitigation and name it* when it appears.
...
- If a technical debate is looping (same arguments, third pass) → stop the loop: extract the load-bearing disagreement (usually one assumption — see decomposing-ambiguity), check if it's cheaply testable (see brainstorming information-buying); if not, name the owner, set the clock, decide, commit.
...
- In meetings you run, speak last: your opinion voiced early becomes the anchor everyone sands their view against (see brainstorming anchoring; the quiet dissent you needed dies unspoken).
...
- **Leading without authority**: staff/senior ICs own outcomes across teams they don't manage — the tools shrink to influence-only: impeccable decision docs (see technical-writing), demos over arguments (see brainstorming: information-buying — build the thin proof), credibility deposits, and enlisting the actual authority when influence hits its ceiling; pushing rank you don't have spends trust you do.
...
- **Inherited teams and predecessors' ghosts**: the decision protocol, quality bar, and safety price you inherit were set by someone else — change them explicitly and slowly (a new lead re-deciding everything in month one is thrash; see refactoring: archaeology before surgery applies to human systems too).
...
- **When safety/legality/integrity is the issue**: disagree-and-commit has a floor — data deletion that violates retention law, a security hole shipped knowingly (see security), gamed metrics: these escalate in writing past any commit, and the discomfort is the job; a lead who commits to integrity violations is laundering them.
