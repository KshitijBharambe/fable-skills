...
"auth": "Use when building login/signup, OAuth/OIDC/SSO, sessions vs JWTs, or any code touching tokens/passwords/reset — plus roles, permissions, sharing, multi-tenant isolation, and resource-by-ID (IDOR) enforcement — RBAC, ABAC, ReBAC, fail-closed.",
...
"brainstorming": "Use when generating ideas (features, designs, architectures, stuck problems) or selecting among options, interrupting the rush to the first plausible idea, or ending a re-litigated decision — inversion, SCAMPER, criteria-first scoring, pre-mortems, kill criteria.",
...
"refactoring": "Use when code's shape resists a change, the same bug family recurs, before extending code you don't understand, changing systems with thin tests and departed authors, running any migration (database, framework, v1-to-v2), or when 'let's just rewrite it' comes up.",
...
"security": "Use when designing any endpoint/form/upload/webhook touching untrusted input, threat-modeling features (money, PII, auth, files), fixing a pentest finding by class, handling a created or leaked credential, adding/auditing dependencies, or designing CI/CD secret handling.",
...
"testing-strategy": "Use when deciding what to test at which layer (unit/integration/E2E), the suite is slow, flaky, or distrusted, coverage is high but bugs ship anyway, reviewing tests that mock everything, or after an escaped bug — pyramid shape, deny paths, flake policy.",
"accessibility": "Use when building or auditing any interactive UI (forms, modals, menus, custom widgets), replacing native elements, handling a WCAG/audit/VPAT requirement, or fixing keyboard/screen-reader/contrast issues — semantics first, ARIA last, focus management.",
"resilience-engineering": "Use when any service calls another service/DB/API, designing failure behavior for a dependency, after a cascade or retry-storm outage, or reviewing network calls with no timeout — timeouts, retries with jitter, circuit breakers, backpressure, graceful degradation.",
"data-modeling": "Use when designing schemas for a new feature or service, choosing normalization vs denormalization, modeling money/time/hierarchies/history, or fixing duplicates, orphans, and impossible states — constraints, identity, temporal data, evolution.",
"domain-modeling": "Use when building business logic (pricing, eligibility, lifecycles, workflows), the same rule lives in three drifting places, controllers grew hundreds of ifs, or 'impossible state' bugs recur — value objects, state machines, invariants, where logic lives.",
"ux-fundamentals": "Use when designing any flow or screen, users 'don't get it' or features go unused, structuring navigation/IA, or adjudicating UX debates — mental models, cognitive load, error prevention, usability heuristics, five-user tests.",
"layout-and-spacing": "Use when laying out screens, a UI 'looks off' or cluttered, defining spacing tokens/grids/breakpoints, reviewing responsive behavior, or content breaks the layout (long names, translations, empty lists) — proximity, alignment, fluid-first responsive design.",
"deployment": "Use when designing how a service ships to production, deploys are scary/manual/rare, choosing rolling vs blue-green vs canary, coordinating schema changes with code, or introducing feature flags and rollback — zero-downtime, expand-migrate-contract, progressive delivery.",
"compliance-and-privacy": "Use when the product touches personal data, payments, or minors, entering GDPR/CCPA territory, handling deletion/export requests, facing SOC 2 or security questionnaires, checking dependency licenses, or shipping through app-store policy gates — data maps, retention, consent, audit trails.",
}
