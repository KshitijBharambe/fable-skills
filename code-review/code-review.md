...
- **Review is a trust-and-knowledge protocol, not a gate with a human face.** Calibrate to the author (the new hire gets depth *and* warmth — their first reviews set their model of the team's bar; the veteran in their home module gets speed), let review rotate so knowledge spreads (see technical-leadership bus factor: review as the cheapest pairing), and remember both directions carry information — the reviewer who only ever transmits learns nothing, and half of review's compound value is the reviewer discovering how this module actually works (see refactoring archaeology: reviews are how you keep the map current). Tone compounds too: the codebase outlives every PR, but so does the relationship — critique the code, never the coder, and write like it'll be read at 2am by someone having a bad week, because it will.
...
3. **Sweep the hot spots**: money paths, auth/authz (see auth: is the object-level check here?), migrations and anything touching data irreversibly, concurrency (shared state, check-then-act — see concurrency-bugs), external contracts (API/event schemas — see api-design; event-driven), injection surfaces (see security: string-built anything), error/retry paths (see async-processing: is it idempotent?).
...
5. **Read the tests as claims**: do they pin the behavior that matters (see testing-strategy), or do they mock the world and assert the mocks? Would they fail if the bug existed? A diff whose tests couldn't catch its own regression is half-reviewed by definition.
...
- The migration file at the bottom of the diff is the most dangerous text in most PRs — read it first, not last (irreversibility lives there; see refactoring expand-contract; postgres locks).
...
- **Generated and vendored diffs**: lockfiles, codegen, formatter migrations — review the *generator inputs* and spot-check outputs; line-reviewing 10k generated lines is theater (but the lockfile diff gets the supply-chain eye — see security: read the lockfile diff).
...
- **Config, infra, and one-line diffs**: the smallest diffs carry the largest blast radii (the retry count, the timeout, the IAM wildcard — see security least-privilege; system-design: config as code) — one-line changes to production behavior get *more* scrutiny per line than anything else in the queue.
...
- Automate ruthlessly and continuously: every human style comment becomes a lint rule the same week; formatting, imports, and obvious footguns (see security: the grep-able banlist) run pre-review — the human budget is reserved for judgment by *construction*.
...
- Use checklists per danger zone, not per PR: the migration checklist (locks? backfill? rollback? — see postgres; refactoring), the endpoint checklist (authz? rate limit? injection? — see security), pulled out only when the diff touches that zone — targeted ceremony beats uniform ceremony.
